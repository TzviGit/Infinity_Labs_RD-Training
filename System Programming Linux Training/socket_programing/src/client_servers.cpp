...
int UDPsocket::SendMSG(std::string buffer, int flags, sockadrr_in *sendTo)
{
    ...
    std::size_t msglen = buffer.length() +1;
...
int UDPsocket::RecMSG(std::string &buffer, std::size_t nbytes, int flags, sockadrr_in *senderAdrr,
               socklen_t *len)
{
    char *msgbuf = const_cast<char *>(buffer.c_str());
    sockaddr *sender = reinterpret_cast<sockaddr *> (senderAdrr);

    int n = recvfrom(m_sockfd, msgbuf, nbytes, flags, sender, len);
    buffer = msgbuf;

...
static const size_t NUM_OF_JOBS = 20;

static const unsigned int g_prog_attempts = 2;
static const unsigned int g_prog_interval = 2;
...
static void SendTerminations()
{
	...
	for (i = 0; i < NUM_OF_INTERRUPTS; i++)
	{
		...
		sleep(delay);
		printf(_PURPLE_ "-- SIGTERM ARRIVED to WD! --" _WHITE_ "\n");
		pid = GetPid(g_file_name_wd_pid);
		kill(pid, SIGTERM);
	}
	...
	sleep(delay);
	printf(_PURPLE_ "-- SIGTERM ARRIVED to program! --\n" _WHITE_ "");
	pid = GetPid(g_file_name_prog_pid); /* need to change to wd */
	kill(pid, SIGTERM);
...
static int RunWorker()
{
	...
	{
		printf(_YELLOW_ "job %d with pid # %d\n" _WHITE_, i , getpid());
		sleep(1);
	}
